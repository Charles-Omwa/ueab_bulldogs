    ...
    if (formValidateAndSave()) {
      try {
        if (_formType == FormType.login) {
          String userId = await widget.auth.signIn(_email.trim(), _password);
          dialogBox.information(
              context, "Congratulations", "You are logged in successfully");
          print(userId);
        } else {
          String userId = await widget.auth.signUp(_email.trim(), _password);
  ...
  List<Widget> createInputs() {
    return [
      TextFormField(style: TextStyle(color: Colors.black),
        decoration: InputDecoration( border: new UnderlineInputBorder(
